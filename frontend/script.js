...
async function getRandomLesson(category = null) {
  try {
    if (category) {
      ...
      const available = lessons.filter(l => !isLessonRead(l));
      ...
      for (const cat of categoriesData.categories) {
        ...
        if (lessons && Array.isArray(lessons)) {
          allLessons.push(...lessons.filter(l => !isLessonRead(l)));
        }
      }
...
const categoryImages = {
  'Animals & Wildlife':         'Animals&Nature.png',
  'Evolution & Prehistory':     'Biology.png',
  'Plants & Ecology':           'Earth&Environment.png',
  'Human Biology':              'Biology.png',
  'Microbiology & Biotech':     'Science.png',
  'Brain & the Senses':         'Philosophy&Consciousness.png',
  'Psychology & Behaviour':     'Philosophy&Consciousness.png',
  'Philosophy & Consciousness': 'Philosophy&Consciousness.png',
  'Medicine & Disease':         'Medicine&Health.png',
  'Physics & Mathematics':      'Science.png',
  'Chemistry':                  'Science.png',
  'AI & Computing':             'Technology&Innovation.png',
  'Engineering & Architecture': 'Technology&Innovation.png',
  'Inventions & Technology':    'Technology&Innovation.png',
  'Ancient History':            'History&Exploration.png',
  'Wars & Exploration':         'History&Exploration.png',
  'Religion & Belief':          'Culture&Arts.png',
  'Arts & Culture':             'Culture&Arts.png',
  'Society & Economics':        'Society&Economics.png',
  'Geography & Natural Wonders':'Earth&Environment.png',
  'Climate & Disasters':        'Earth&Environment.png',
  'Oceans & Marine Life':       'Marine&Ocean.png',
  'Space & Cosmos':             'Space&Cosmos.png',
  'Mysteries & Records':        'Mysteries&Unexplained.png',
};

// Per-category colour theme
const CATEGORY_COLORS = {
  'Animals & Wildlife':         { bg: '#dcfce7', color: '#16a34a' },
  'Evolution & Prehistory':     { bg: '#bbf7d0', color: '#15803d' },
  'Plants & Ecology':           { bg: '#d1fae5', color: '#047857' },
  'Human Biology':              { bg: '#ccfbf1', color: '#0f766e' },
  'Microbiology & Biotech':     { bg: '#cffafe', color: '#0e7490' },
  'Brain & the Senses':         { bg: '#e0e7ff', color: '#3730a3' },
  'Psychology & Behaviour':     { bg: '#ede9fe', color: '#6d28d9' },
  'Philosophy & Consciousness': { bg: '#ddd6fe', color: '#5b21b6' },
  'Medicine & Disease':         { bg: '#fee2e2', color: '#b91c1c' },
  'Physics & Mathematics':      { bg: '#fff9c4', color: '#a16207' },
  'Chemistry':                  { bg: '#fef3c7', color: '#b45309' },
  'AI & Computing':             { bg: '#dbeafe', color: '#1e40af' },
  'Engineering & Architecture': { bg: '#e0f2fe', color: '#0369a1' },
  'Inventions & Technology':    { bg: '#bfdbfe', color: '#1d4ed8' },
  'Ancient History':            { bg: '#fef3c7', color: '#92400e' },
  'Wars & Exploration':         { bg: '#ffedd5', color: '#c2410c' },
  'Religion & Belief':          { bg: '#fce7f3', color: '#9d174d' },
  'Arts & Culture':             { bg: '#fdf2f8', color: '#be185d' },
  'Society & Economics':        { bg: '#fff7ed', color: '#c2410c' },
  'Geography & Natural Wonders':{ bg: '#ecfdf5', color: '#065f46' },
  'Climate & Disasters':        { bg: '#f0fdf4', color: '#166534' },
  'Oceans & Marine Life':       { bg: '#eff6ff', color: '#1d4ed8' },
  'Space & Cosmos':             { bg: '#f5f3ff', color: '#5b21b6' },
  'Mysteries & Records':        { bg: '#fdf4ff', color: '#7e22ce' },
};
...
async function loadCategoryLessonsView(categoryId, categoryName) {
  ...
  try {
    ...
    const rows = lessons.map((lesson, idx) => {
      ...
      const tickSvg = `<svg class="lesson-read-tick" viewBox="0 0 24 24"><circle cx="12" cy="12" r="11" fill="#ecfdf5"/><polyline points="7 12 10.5 15.5 17 9" fill="none" stroke="#10b981" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
      const thumbHtml = imgUrl
        ? `<div class="lesson-thumb-wrap"><img class="lesson-thumb" src="${imgUrl}" alt="" loading="lazy" onerror="this.parentElement.remove()">${read ? tickSvg : ''}</div>`
        : (read ? `<div style="position:relative;width:14px;flex-shrink:0;">${tickSvg}</div>` : '');
      return `
      <div class="explore-category-row${read ? ' is-read' : ''}" data-lesson-idx="${idx}" data-accent="${color}" onclick="selectLessonFromCard('${categoryId}', ${idx})">
        <div class="lesson-accent-bar" style="background:${color};"></div>
        <div class="explore-category-info">
          <div class="explore-category-name">${cleanTitle(lesson.title, lesson.topic)}</div>
          <div class="explore-category-desc">${preview}${preview.length >= 100 ? '…' : ''}</div>
        ...
        ${thumbHtml}
        ${!read ? `<svg class="lesson-chevron" viewBox="0 0 24 24" fill="none" stroke="${color}" stroke-width="2.5" style="width:18px;height:18px;"><polyline points="9 18 15 12 9 6"/></svg>` : ''}
      </div>`;
    }).join('');
...
function closeFullLesson() {
  ...
  if (lesson && isLessonRead(lesson)) {
    ...
    if (row) {
      ...
      // Move chevron → tick overlaid on thumbnail
      const chevron = row.querySelector('.lesson-chevron');
      if (chevron) chevron.remove();
      const wrap = row.querySelector('.lesson-thumb-wrap');
      const tickSvg = `<svg class="lesson-read-tick" viewBox="0 0 24 24"><circle cx="12" cy="12" r="11" fill="#ecfdf5"/><polyline points="7 12 10.5 15.5 17 9" fill="none" stroke="#10b981" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
      if (wrap && !wrap.querySelector('.lesson-read-tick')) wrap.insertAdjacentHTML('beforeend', tickSvg);
    }
  }
